...
export default function WarehouseList() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await api.put(`/warehouse/warehouses/${editingId}`, form);
      } else {
      ...
      alert(err?.response?.data?.message || "Lỗi khi lưu kho.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 w-full">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500">Chọn một kho để xem tồn kho của kho đó</p>
        </div>
      ...
      {showForm && (
        <div className="mb-6 bg-white border border-neutral-200 rounded-lg p-6">
          ...
          <form onSubmit={handleSave} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-neutral-500 mb-1">Tên kho *</label>
            ...
            <div>
              <label className="block text-xs text-neutral-500 mb-1">Mã kho</label>
            ...
            <div className="sm:col-span-2">
              <label className="block text-xs text-neutral-500 mb-1">Địa điểm</label>
      ...
      {loading ? (
        ...
        <div className="border border-dashed border-neutral-200 rounded-lg bg-white p-12 text-center">
          <p className="text-sm text-neutral-500">Chưa có kho nào.</p>
          <button type="button" onClick={openCreate}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-neutral-50 text-left text-xs uppercase tracking-wider text-neutral-500">
                  ...
                  <th className="px-4 py-3 font-medium hidden sm:table-cell">Địa điểm</th>
                  <th className="px-4 py-3 font-medium hidden md:table-cell">Tổng tồn</th>
                  <th className="px-4 py-3 font-medium hidden lg:table-cell">Trạng thái</th>
              ...
              <tbody>
                {warehouses.map((w) => (
                  <tr key={w.id} className="border-t border-neutral-100 hover:bg-neutral-50/80 transition-colors">
                    ...
                    <td className="px-4 py-3 text-neutral-600 hidden sm:table-cell">{w.location || "—"}</td>
                    <td className="px-4 py-3 text-neutral-600 hidden md:table-cell">{w.total_stock ?? 0}</td>
                    <td className="px-4 py-3 hidden lg:table-cell">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${
                        w.is_active ? "text-green-600 bg-green-50" : "text-neutral-500 bg-neutral-100"
                      }`}>
