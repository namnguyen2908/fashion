...
function ConfirmDialog({ open, title, message, submitting, onCancel, onConfirm, confirmLabel = "Xác nhận" }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={onCancel}>
      <div className="bg-white shadow-2xl rounded-lg border border-neutral-100 p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-base font-medium mb-2">{title}</h3>
        <p className="text-sm text-neutral-600 mb-6">{message}</p>
        <div className="flex justify-end gap-3">
          <button type="button" onClick={onCancel} disabled={submitting} className="px-4 py-2 text-sm text-neutral-600">
            Hủy
          </button>
          <button
            type="button"
            onClick={onConfirm}
            disabled={submitting}
            className="inline-flex items-center gap-2 px-4 py-2 text-sm bg-black text-white rounded-md hover:bg-neutral-800 disabled:opacity-60"
          >
...
export default function ProductDetail() {
  ...
  const [newVariant, setNewVariant] = useState({ color: "", size: "", price: "", list_price: "" });
  ...
  const addVariant = async (e) => {
    ...
    if (!newVariant.price || Number(newVariant.price) <= 0) {
      setError("Giá bán biến thể mới phải hợp lệ.");
      return;
    }
    ...
    try {
      ...
      setNewVariant({ color: "", size: "", price: "", list_price: "" });
        ...
        <div className="space-y-6">
                ...
                <tbody>
                  {variants.map((v) => (
                    <tr key={v.id} className="border-t border-neutral-100">
                      ...
                      <td className="px-3 py-2">
                        <input type="number" min="1" value={variantEdits[v.id]?.price ?? ""} onChange={(e) => updateVariantEdit(v.id, "price", e.target.value)} className="w-28 border border-neutral-200 rounded px-2 py-1 text-sm" />
                      </td>
                      <td className="px-3 py-2">
          ...
          <form onSubmit={addVariant} className="bg-white border border-neutral-200 rounded-lg p-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <input type="number" placeholder="Giá gốc" value={newVariant.list_price} onChange={(e) => setNewVariant((v) => ({ ...v, list_price: e.target.value }))} className={inputClass} />
            </div>
